...
describe("/categories", () => {
  ...
  it("POST /categories -  Must be able to create category", async () => {
  ...
  it("POST /categories -  should not be able to list users not being admin", async () => {
  ...
  it("GET /categories -  Must be able to list all categories", async () => {
    const response = await request(app).get("/categories");
    expect(response.body).toHaveLength(1);
    expect(response.status).toBe(200);
  });

  it("GET /categories/:id/properties -  Must be able to list one category properties", async () => {
